...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/10 20:15:07 by gwaymar-          #+#    #+#             */
/*   Updated: 2019/07/14 19:54:01 by gwaymar-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void		ship_compl(t_fract_compl f, int *i, int max_it)
{
	*i = -1;
	while (++(*i) < max_it)
	{
		f.oldre = f.newre;
		f.oldim = f.newim;
		f.newre = f.oldre * f.oldre - f.oldim * f.oldim + f.r;
		f.newim = 2 * ft_abs_double(f.oldim * f.oldre) + f.i;
...
static t_color	go_ship(int x, int y, t_mlx *mlx)
{
	t_color			color;
	int				i;
	t_fract_compl	f;
