...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/10 20:37:29 by gwaymar-          #+#    #+#             */
/*   Updated: 2019/07/14 19:55:56 by gwaymar-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void		trio_compl(t_fract_compl f, int *i, int max_it)
{
	*i = -1;
	while (++(*i) < max_it)
	{
		f.oldre = f.newre;
		f.oldim = f.newim;
		f.newre = f.oldre * f.oldre * f.oldre * f.oldre + f.oldim *
			f.oldim * f.oldim * f.oldim -
			6 * f.oldre * f.oldre * f.oldim * f.oldim + f.r;
		f.newim = 4 * f.oldre * f.oldim * (f.oldre * f.oldre -
				f.oldim * f.oldim) + f.i;
...
static t_color	go_trio(int x, int y, t_mlx *mlx)
{
	t_color			color;
	int				i;
	t_fract_compl	f;
